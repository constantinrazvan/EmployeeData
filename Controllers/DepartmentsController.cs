...
public class DepartmentsController : Controller
{
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Id,Name,Code,Description,ManagerName")] Department department)
    {
        ...
        var emps = await _context.Persons
            .Select(p => p.FirstName + " " + p.LastName)
            .OrderBy(name => name)
            .ToListAsync();
        if (!string.IsNullOrEmpty(department.ManagerName) && !emps.Contains(department.ManagerName))
        {
            emps.Insert(0, department.ManagerName);
        }
