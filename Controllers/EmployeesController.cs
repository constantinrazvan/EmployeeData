...
public class EmployeesController : Controller
{
    ...
    public async Task<IActionResult> Index(string searchTerm, int? departmentId, string status, int pageNumber = 1)
    {
        ...
        var employees = await query.OrderBy(p => p.LastName).ThenBy(p => p.FirstName)
    ...
    public async Task<IActionResult> Delete(int? id)
    {
        ...
        var person = await _context.Persons.Include(p => p.Department).FirstOrDefaultAsync(m => m.Id == id);
        if (person == null) return NotFound();
    ...
    public async Task<IActionResult> ExportCsv(string searchTerm, int? departmentId, string status)
    {
        ...
        if (!string.IsNullOrEmpty(searchTerm))
        {
            ...
            query = query.Where(p => p.FirstName.ToLower().Contains(s) || p.LastName.ToLower().Contains(s) || p.Email.ToLower().Contains(s) || p.Position.ToLower().Contains(s));
        }
        ...
        return File(encoding.GetBytes(sb.ToString()), "text/csv; charset=utf-8", $"employees_{DateTime.Now:yyyyMMdd}.csv");
    }
